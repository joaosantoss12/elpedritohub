...
function CustomDot(props: DotProps) {
  ...
  const color =
    ...
    payload.aposta.estado === 'perdida' ? '#f87171' : '#9aa7bd';
  return <circle cx={cx} cy={cy} r={6} fill={color} stroke="rgba(255, 255, 255, 0.9)" strokeWidth={2} />;
}
function CustomActiveDot(props: DotProps) {
  const { cx, cy, payload } = props;
  if (cx === undefined || cy === undefined) return null;
  const color = !payload?.aposta ? '#a17c5b' :
    payload.aposta.estado === 'ganha' ? '#4ade80' :
    payload.aposta.estado === 'perdida' ? '#f87171' : '#9aa7bd';
  return <circle cx={cx} cy={cy} r={9} fill={color} stroke="rgba(255, 255, 255, 0.9)" strokeWidth={2} />;
}
                    ...
                    <AreaChart data={bucketData} margin={{ top: 8, right: 16, left: 0, bottom: 0 }}>
                      <defs>
                        <linearGradient id="bkGrad" x1="0" y1="0" x2="0" y2="1">
                          <stop offset="5%" stopColor="#a17c5b" stopOpacity={0.22} />
                          <stop offset="95%" stopColor="#a17c5b" stopOpacity={0} />
                        </linearGradient>
                      </defs>
                      ...
                      <Area
                        type="monotone" dataKey="cum" stroke="#a17c5b" strokeWidth={2}
                        fill="url(#bkGrad)" dot={false}
                        activeDot={{ r: 5, fill: '#a17c5b', stroke: '#1a2233', strokeWidth: 2 }}
                      />
                    </AreaChart>
                  </ResponsiveContainer>
                ) : (
                      ...
                      <AreaChart data={dayData} margin={{ top: 10, right: 16, left: 0, bottom: 0 }}>
                        <defs>
                          <linearGradient id="dayGrad" x1="0" y1="0" x2="0" y2="1">
                            <stop offset="5%" stopColor="#a17c5b" stopOpacity={0.25} />
                            <stop offset="95%" stopColor="#a17c5b" stopOpacity={0} />
                          </linearGradient>
                        </defs>
                        <CartesianGrid strokeDasharray="3 3" stroke="rgba(42,33,24,0.08)" />
                        <XAxis dataKey="label" tick={{ fill: '#6b7891', fontSize: 11 }} axisLine={false} tickLine={false} />
                        <YAxis domain={yDomain} tick={{ fill: '#6b7891', fontSize: 11 }} axisLine={false} tickLine={false} tickFormatter={v => `€${v}`} width={55} />
                        <Tooltip content={<BetTooltip />} />
                        <ReferenceLine y={0} stroke="rgba(42,33,24,0.18)" strokeDasharray="4 4" />
                        <Area
                          type="monotone" dataKey="cum" stroke="#a17c5b" strokeWidth={2} fill="url(#dayGrad)"
